...
const NavBar = () => {
  ...
  return (
    <div className="flex justify-between items-center w-full px-4 text-white bg-black fixed ">
        <div>
            <h1 className="text-3xl ml-2 mt-2">LOGO</h1>
        </div>
      ...
      {nav && (
        ...
        {links.map(({id, link}) => (
            <li key={id} className='px-4 cursor-pointer capitalize py-6 text-4xl hover:scale-105 duration-200 text-gray-500 hover:text-white'>{link}</li>
          )
